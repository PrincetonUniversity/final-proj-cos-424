...
G1 = MVN(np.identity(2), np.array([10, 0]))
G2 = MVN(np.array([[1.0, 0.0], [0.0, 1.0]]), np.zeros(2))
...
ys = [-10, 0, 2.5, 10, 20]
pts = np.arange(-5, 5, 0.1, dtype='float64').reshape(-1, 1)
...
ys = np.arange(-5, 5, 0.1, dtype='float64').reshape(-1, 1)
mm, sm, ams = marginal_posterior(ys, mr, sr, a)
...
xs3 = [f(i, am) for i, am in enumerate(ams[lo:hi])]

print(xs3)

import pickle

with open('xs3-{}-{}.p'.format(lo, hi), 'wb') as w:
    pickle.dump(xs3, w)
